				...
				<?php if ( have_posts() ) { ?>
					<div class="row top-items">
						<div class="col-md-6 col-sm-6 col-xs-12 posts-found">
							...
							<?php esc_html_e( 'posts found', 'foodrecipe' ); ?>
					...
					<?php while ( have_posts() ) {
						...
						<div <?php post_class(); ?> >
							...
							<?php if ( has_tag() ) { ?>
								<div class='tags-div'><?php the_tags(); ?></div>
							<?php } ?>
							<div class="button-div">
								<?php if ( 'image' == get_post_format() || 'gallery' == get_post_format() ){ ?>
									<a href="<?php the_permalink(); ?>">
										<button type="button" class="read-more-post">
											<?php esc_html_e( 'view more', 'foodrecipe' ); ?>
										</button>
									</a>
								<?php } elseif ( 'video' != get_post_format() ) { ?>
									<a href="<?php the_permalink(); ?>">
							 			<button type="button" class="read-more-post">
							 				<?php esc_html_e( 'read more', 'foodrecipe' ); ?>
							 			</button>
						 			</a>
								<?php } ?>
							</div><!-- .button-div -->
